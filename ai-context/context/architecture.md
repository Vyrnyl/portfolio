...
- `sitemap.ts` and `robots.ts` are generated from the content layer — a new project appears in the sitemap automatically. **Built in PORT-051.** The six STATIC routes are listed literally in `sitemap.ts`, and that is deliberate: Next has no route-enumeration API, so generating the list means globbing for `page.tsx`, which sweeps up `/gallery` and the five `opengraph-image` PNG endpoints — a glob would need a deny-list to be correct, making it a hand-written list with extra failure modes. The trade is named in the file: **a new page must be added there by hand.** Project entries are mapped from `getAllProjects()`, so those really are automatic. `robots.ts` disallows `/gallery` but deliberately does NOT disallow the card routes — a crawler must be able to fetch an `og:image`, and blocking them would break every link preview while looking like housekeeping.
- JSON-LD `Person` + `WebSite` in the root layout, `CreativeWork` on project detail pages, all built by `lib/structured-data.ts` (PORT-051). The three nodes share `@id` anchors (`#person`, `#website`) so a project's `author` REFERENCES the one Person rather than restating it — without that, every project page declares an anonymous author and nothing links them. Scripts render in `<body>`, outside `ThemeProvider`, since their content is a build-time constant with no reason to enter a client subtree. Emitted fields were Vernel's call: name, role, url and `sameAs` (GitHub + LinkedIn, `mailto:` filtered out), with email and location deliberately omitted — both are already visible on `/contact`, but a JSON-LD `email` is a materially easier harvest than a rendered `mailto:`. Nothing is emitted that the page does not state: no `datePublished` (project `year` is inferred, not a stated publication date) and no `image` (every screenshot is still a placeholder).
- **A placeholder filter must cover every TYPE of field it guards, not every field of one type.** PORT-050 learned that the card and the description are separate surfaces; PORT-051 found the same class a third time in a different shape. `isPlaceholder` matches marker WORDS — TBC, placeholder, pending — and `projects.ts` holds `https://example.com` as a stand-in URL, which contains none of them. It passed straight into `sameAs`, the strongest claim in the node: an assertion that the project IS the thing at that URL. Caught by reading the served JSON-LD. URLs now go through a separate `isPlaceholderUrl` on RFC 2606 reserved names.
