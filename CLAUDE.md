...
Next ticket: **PORT-036** (Contact page UI) or **PORT-037** (Skills) — both unblocked, Vernel's pick. Still carried and now visible on six real pages: root `layout.tsx` has hardcoded metadata (`“— Developer”`, not `site.role`) and **no `metadataBase`**, flagged since PORT-030; and `global-error.tsx` still does not exist.
