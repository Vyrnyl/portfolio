...
Next ticket: **PORT-022** (Badge). It has a waiting debt already: `ProjectCard`'s tag chips are inlined with the exact classes Badge should ship with — sweep that one spot in the same ticket. Open question 7 (`coral` on `ground` measures 3.97:1 in light mode, below AA) is still unanswered and gates PORT-023 — settle it before Sprint 2 ends.
