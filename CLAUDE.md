...
Next ticket: **PORT-043** (spam and rate limiting), deliberately out of dependency order — four of its five criteria need no Resend, and the fifth is already met by PORT-041. **PORT-042 is parked by choice, not blocked**, and open question 3 (the sending domain) is still unanswered. Do not pull PORT-044 forward with it: its success copy needs a real send behind it. Still carried and visible on all seven pages: root `layout.tsx` has hardcoded metadata (`“— Developer”`, not `site.role`) and **no `metadataBase`**, flagged since PORT-030; `global-error.tsx` still does not exist; and `/contact` still tells visitors "it reached my inbox" when nothing is sent.
