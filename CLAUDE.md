...
Next ticket: **PORT-034** (Resume). Depends on 015 and 022, both closed — but **the “resume PDF updated” prerequisite is still `☐`**, and `public/resume.pdf` is what the page links to as a download, so settle that with Vernel first. `Timeline` is still `designed` in the registry: build to that entry. Note `jobs[0].bullets` are three literal `Placeholder` strings, so the timeline would put placeholder text on a real page for the first time — his call whether that ships or PORT-057 is pulled forward for that one job.
