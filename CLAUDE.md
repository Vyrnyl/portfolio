...
Next ticket: **PORT-037** (Skills) — the last page in Sprint 3. Note it carries a content-writing step: `SkillGroup.tier` and the `Practice` type exist and `getSkillGroups()` already sorts the tiers, but **no `practices` array has ever been written** and `lib/content.ts` has no accessor for it, so four practice cards of original prose are part of that ticket. Still carried and now visible on seven real pages: root `layout.tsx` has hardcoded metadata (`“— Developer”`, not `site.role`) and **no `metadataBase`**, flagged since PORT-030; and `global-error.tsx` still does not exist.
