...
Next ticket: **PORT-040** (validation schema), the first of Sprint 4 — contact wiring. `ContactForm`'s `FormState` was already shaped like §6's `ActionResult`, so PORT-041 swaps `useState` → `useActionState` and deletes `mockValidate` without touching markup. Still carried and now visible on all seven pages: root `layout.tsx` has hardcoded metadata (`“— Developer”`, not `site.role`) and **no `metadataBase`**, flagged since PORT-030; and `global-error.tsx` still does not exist.
